...
const ytdAdjustmentReport = seedData.dailyReports.find((report) => report.date === '2026-07-28')
assert(ytdAdjustmentReport, 'Year-to-date adjustment report for 2026-07-28 should exist')
...
assert.equal(ytdAdjustmentTotals.totalRevenue, 81179, 'YTD adjustment report should fill the missing 81.179 kr.')
assert.equal(ytdAdjustmentTotals.totalItems, 2739, 'YTD adjustment report should fill the missing 2.739 items')
...
const yearToDateSummary = calculateDateRangeSummary(seedData, '2026-01-01', '2026-07-28')
assert.equal(yearToDateSummary.totalRevenue, 90070, '2026 year-to-date summary should match POS revenue')
assert.equal(yearToDateSummary.totalItems, 2984, '2026 year-to-date summary should match POS item count')
...
assert(normalizeData({ dailyReports: [exampleReport] }).dailyReports.some((report) => report.date === '2026-07-28'), 'YTD adjustment should migrate into existing saved data')
assert(!normalizeData({ dailyReports: [{ ...ytdAdjustmentReport, date: '2026-07-14', items: [{ productId: 'guf', quantity: 240 }] }] }).dailyReports.some((report) => report.date === '2026-07-14' && report.id === 'report-2026-ytd-adjustment'), 'Old POS adjustment should be replaced by the latest seed adjustment')
...
assert.equal(calculateStock(gufStock, seedData.dailyReports).currentStock, -397, 'Guf stock should follow known Guf sales')
assert.equal(
...
assert.equal(calculateStock(chocolateStock, seedData.dailyReports, seedData.stockMovements).currentStock, 12, 'Chocolate purchase stock should include July restocks')
...
assert.equal(calculateStock(vanillaStock, seedData.dailyReports, seedData.stockMovements).currentStock, 11, 'Vanilla restock stock should include July restocks')
...
assert.equal(calculateStock(softiceMixStock, seedData.dailyReports, seedData.stockMovements).currentStock, 22, 'Softice Mix stock should include all received kolli')
...
assert.equal(calculateStock(friskoPistacieStock, seedData.dailyReports, seedData.stockMovements).currentStock, 5, 'Frisko Pistacie stock should show 5 received kolli')
...
assert.equal(calculateStock(cookieDoughStock, seedData.dailyReports, seedData.stockMovements).currentStock, 3, 'B&J Cookie Dough stock should show 3 received kolli')
