...
const ytdAdjustmentReport = seedData.dailyReports.find((report) => report.date === '2026-08-12')
assert(ytdAdjustmentReport, 'Year-to-date adjustment report for 2026-08-12 should exist')
...
assert.equal(ytdAdjustmentTotals.totalRevenue, 117356, 'YTD adjustment report should fill the missing 117.356 kr.')
assert.equal(ytdAdjustmentTotals.totalItems, 4028, 'YTD adjustment report should fill the missing 4.028 items')
...
const yearToDateSummary = calculateDateRangeSummary(seedData, '2026-01-01', '2026-08-12')
assert.equal(yearToDateSummary.totalRevenue, 126247, '2026 year-to-date summary should match tracked POS ice cream revenue')
assert.equal(yearToDateSummary.totalItems, 4273, '2026 year-to-date summary should match tracked POS ice cream item count')
approx(yearToDateSummary.expenses, 68074.63, '2026 year-to-date summary should include product purchases and paycheck')
approx(yearToDateSummary.netExpenses, 55459.704, '2026 year-to-date net expenses should include product purchases ex. moms and full paycheck')
assert(yearToDateSummary.productBreakdown.some((entry) => entry.product === 'Flødebolle' && entry.quantity === 3 && entry.revenue === 21), 'YTD summary should include Flødebolle')
...
assert(normalizeData({ dailyReports: [exampleReport] }).dailyReports.some((report) => report.date === '2026-08-12'), 'YTD adjustment should migrate into existing saved data')
assert(!normalizeData({ dailyReports: [{ ...ytdAdjustmentReport, date: '2026-07-29', items: [{ productId: 'guf', quantity: 422 }] }] }).dailyReports.some((report) => report.date === '2026-07-29' && report.id === 'report-2026-ytd-adjustment'), 'Old POS adjustment should be replaced by the latest seed adjustment')
...
assert(normalizeData({ expenses: [] }).expenses.some((expense) => expense.id === 'expense-unilever-restock-2026-08-12'), '12/08 Unilever restock expense should migrate into existing saved data')
...
assert(normalizeData({ stockItems: [] }).stockItems.some((item) => item.id === 'stock-bj-cookie-dough-4-5l-x2'), '17/07 restock stock items should migrate into existing saved data')
assert(normalizeData({ stockItems: [] }).stockItems.some((item) => item.id === 'stock-cdo-banana-chocolate-5-5l'), '12/08 restock stock items should migrate into existing saved data')
...
assert(normalizeData({ stockMovements: [] }).stockMovements.some((movement) => movement.id === 'movement-ice-cream-restock-cdo-vanilla-5l-2026-07-14'), 'July restock stock movements should migrate into existing saved data')
assert(normalizeData({ stockMovements: [] }).stockMovements.some((movement) => movement.id === 'movement-unilever-restock-bj-cookie-dough-4-5l-x2-2026-07-17'), '17/07 restock stock movements should migrate into existing saved data')
assert(normalizeData({ stockMovements: [] }).stockMovements.some((movement) => movement.id === 'movement-unilever-restock-cdo-banana-chocolate-5-5l-2026-08-12'), '12/08 restock stock movements should migrate into existing saved data')
...
const julyPurchaseExpenses = seedData.expenses.filter((expense) => expense.date === '2026-07-14' && expense.type === 'Ice cream purchase')
approx(julyPurchaseExpenses.reduce((sum, expense) => sum + expense.amount, 0), 17563.57, 'July purchases should total 17.563,57 kr. incl. moms')
const julyPurchaseMovements = seedData.stockMovements.filter((movement) => movement.date === '2026-07-14')
assert.equal(julyPurchaseMovements.length, 13, 'July purchases should create one received movement per invoice line')
assert.equal(julyPurchaseMovements.reduce((sum, movement) => sum + movement.quantity, 0), 49, 'July purchases should receive 49 kolli')
const july17PurchaseExpense = seedData.expenses.find((expense) => expense.id === 'expense-unilever-restock-2026-07-17')
assert(july17PurchaseExpense, '17/07 Unilever restock expense should exist')
approx(july17PurchaseExpense.amount, 17681.6, '17/07 Unilever restock should be entered incl. moms')
...
const august12PurchaseExpense = seedData.expenses.find((expense) => expense.id === 'expense-unilever-restock-2026-08-12')
assert(august12PurchaseExpense, '12/08 Unilever restock expense should exist')
approx(august12PurchaseExpense.amount, 17012.2, '12/08 Unilever restock should be entered incl. moms')
approx(splitExpenseVat(august12PurchaseExpense, seedData.settings).net, 13609.76, '12/08 Unilever restock should calculate net expense ex. moms')
const august12PurchaseMovements = seedData.stockMovements.filter((movement) => movement.date === '2026-08-12')
assert.equal(august12PurchaseMovements.length, 15, '12/08 Unilever restock should create one received movement per invoice line')
assert.equal(august12PurchaseMovements.reduce((sum, movement) => sum + movement.quantity, 0), 42, '12/08 Unilever restock should receive 42 kolli')
...
assert.equal(calculateStock(gufStock, seedData.dailyReports).currentStock, -546, 'Guf stock should follow known Guf sales')
assert.equal(
...
assert.equal(calculateStock(chocolateStock, seedData.dailyReports, seedData.stockMovements).currentStock, 14, 'Chocolate purchase stock should include all restocks')
...
const vanillaStock = seedData.stockItems.find((item) => item.id === 'stock-cdo-vanilla-5l')
assert(vanillaStock, 'Vanilla restock stock item should exist')
assert.equal(calculateStock(vanillaStock, seedData.dailyReports, seedData.stockMovements).currentStock, 17, 'Vanilla restock stock should include all restocks')
const softiceMixStock = seedData.stockItems.find((item) => item.id === 'stock-softice-mix-5l')
assert(softiceMixStock, 'Softice Mix stock item should exist')
assert.equal(calculateStock(softiceMixStock, seedData.dailyReports, seedData.stockMovements).currentStock, 32, 'Softice Mix stock should include all received kolli')
const friskoPistacieStock = seedData.stockItems.find((item) => item.id === 'stock-frisko-pistacie-5l')
assert(friskoPistacieStock, 'Frisko Pistacie stock item should exist')
assert.equal(calculateStock(friskoPistacieStock, seedData.dailyReports, seedData.stockMovements).currentStock, 7, 'Frisko Pistacie stock should include all received kolli')
const cookieDoughStock = seedData.stockItems.find((item) => item.id === 'stock-bj-cookie-dough-4-5l-x2')
assert(cookieDoughStock, 'B&J Cookie Dough stock item should exist')
assert.equal(calculateStock(cookieDoughStock, seedData.dailyReports, seedData.stockMovements).currentStock, 6, 'B&J Cookie Dough stock should include all received kolli')
const bananaChocolateStock = seedData.stockItems.find((item) => item.id === 'stock-cdo-banana-chocolate-5-5l')
assert(bananaChocolateStock, 'CDO Banana & Chocolate stock item should exist')
assert.equal(calculateStock(bananaChocolateStock, seedData.dailyReports, seedData.stockMovements).currentStock, 1, 'CDO Banana & Chocolate stock should show 1 received kolli')
const raspberryWhiteChocolateStock = seedData.stockItems.find((item) => item.id === 'stock-cdo-raspberry-white-chocolate-5-5l')
assert(raspberryWhiteChocolateStock, 'CDO Raspberry & White Chocolate stock item should exist')
assert.equal(calculateStock(raspberryWhiteChocolateStock, seedData.dailyReports, seedData.stockMovements).currentStock, 1, 'CDO Raspberry & White Chocolate stock should show 1 received kolli')
const chocHazelnutStock = seedData.stockItems.find((item) => item.id === 'stock-carte-dor-choc-hazelnut-5-5l')
assert(chocHazelnutStock, "Carte d'Or Choc Hazelnut stock item should exist")
assert.equal(calculateStock(chocHazelnutStock, seedData.dailyReports, seedData.stockMovements).currentStock, 1, "Carte d'Or Choc Hazelnut stock should show 1 received kolli")
